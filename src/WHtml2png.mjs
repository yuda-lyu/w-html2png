    ...
    let exec = async() => {
        ...
        let core = async () => {
            ...
            let supplyHtml = async (fun) => {
                ...
                //fnOut
                let fnOut = `./whpic_${id}.png` //一定要給副檔名, 否則puppeteer的screenshot會無法識別格式

                //fpOut
                // let fpOut = path.resolve(wd, fnOut)
                let fpOut = path.resolve(fnOut)
                // console.log('fpOut', fpOut)

                //fnHtml
                let fnHtml = `./whweb_${id}.html`

                //fpHtml
                // let fpHtml = path.resolve(wd, fnHtml)
                let fpHtml = path.resolve(fnHtml)
...
`
        ...
        await core()
            ...
            .finally(() => {
                ...
                //fnHtml
                let fnHtml = `./whweb_${id}.html`
                if (writeError && fsIsFile(fnHtml)) {
                    fs.writeFileSync(`./_err_${id}_fnHtml.json`, `can not delete html[${fnHtml}]`, 'utf8')
                }

                //fd
                let fd = `./_puppeteer_profile_${id}`
                if (writeError && fsIsFolder(fd)) {
                    fs.writeFileSync(`./_err_${id}_profile.json`, `can not delete profile[${fd}]`, 'utf8')
                }

                //fnOut
                let fnOut = `./whpic_${id}.png`
                if (writeError && fsIsFile(fnOut)) {
                    fs.writeFileSync(`./_err_${id}_fnPng.json`, `can not delete png[${fnOut}]`, 'utf8')
                }
